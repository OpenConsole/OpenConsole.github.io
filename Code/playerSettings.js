...
(function () {
    ...
    if(player.lastServerA != null) {
        recvIdInputA.value = player.lastServerA;
    }
    if(player.lastServerB != null) {
        recvIdInputB.value = player.lastServerB;
    }
    recvIdInputA.onchange = function() {
        player.lastServerA = recvIdInputA.value;
        bake_cookie("Player", player, 1);
    }
    recvIdInputB.onchange = function() {
        player.lastServerB = recvIdInputB.value;
        bake_cookie("Player", player, 1);
    }
    ...
    if(player.nameSet) {
        ...
        playerNameInput.placeholder = "e.g. " + player.name;
    }
...
document.addEventListener("click", function (e) {
    if(e.target && e.target.id == "gamesSelect"){
        e.stopPropagation();
        return;
    }
    openFullscreen();
});
controllerIFrame.contentDocument.addEventListener("click", openFullscreen);
